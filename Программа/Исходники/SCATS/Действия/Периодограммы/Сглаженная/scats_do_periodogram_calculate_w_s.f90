...
implicit none
     ...
     module procedure scats_do_periodogram_calculate_w
          ...
          if ( .not. allocated(result%cw) ) call scats_do_errors_log_error('NA_cw', result=result)
          ...
          if ( allocated(result%Dw) ) then
               ...
               if ( .not. size(result%Dw, kind=JP) .eq. N_1_JP + 1_JP ) then
                    ...
                    if ( stat .ne. 0_SP ) call scats_do_errors_log_error('WD_Dw', result=result)

                    ! Выделение памяти под массив значений сглаженной периодограммы
                    allocate( result%Dw(0_JP:N_1_JP), stat = stat )
                    if ( stat .ne. 0_SP ) call scats_do_errors_log_error('WA_Dw', result=result)
          ...
          if ( allocated(result%v) ) then
               ...
               if ( size(result%v, kind=JP) .ne. N_1_JP + 1_JP ) then
                    ...
                    if ( stat .ne. 0_SP ) call scats_do_errors_log_error('WD_v', result=result)

                    ! Выделение памяти под массив частот периодограммы
                    allocate( result%v(0_JP:N_1_JP), stat = stat )
                    if ( stat .ne. 0_SP ) call scats_do_errors_log_error('WA_v', result=result)
          ...
          if ( stat .ne. 0_SP ) call scats_do_errors_log_error('WA_X_FFT', result=result)
          ...
          if ( stat .ne. 0_SP ) call scats_do_errors_log_error('WD_X_FFT', result=result)
