...
implicit none
     ...
     module procedure scats_gen_generate
          ...
          if (.not. gen%params%ready) then; call scats_gen_log_error('NA', call_stat); return; endif
          ...
          if ( allocated(input%t) ) then
               ...
               if ( .not. size(input%t, kind=IP) .eq. N ) then
                    ...
                    if ( stat .ne. 0_SP ) then; call scats_gen_log_error('WD_t', call_stat); return; endif

                    ! Выделение памяти под массив времени
                    allocate( input%t(0:N_m1_JP), stat = stat )
                    if ( stat .ne. 0_SP ) then; call scats_gen_log_error('WA_t', call_stat); return; endif
          ...
          if ( allocated(input%x) ) then
               ...
               if ( .not. size(input%x, kind=IP) .eq. N ) then
                    ...
                    if ( stat .ne. 0_SP ) then; call scats_gen_log_error('WD_x', call_stat); return; endif

                    ! Выделение памяти под массив значений
                    allocate( input%x(0:N_m1_JP), stat = stat )
                    if ( stat .ne. 0_SP ) then; call scats_gen_log_error('WA_x', call_stat); return; endif
          ...
          if ( add_trend ) then
               ...
               if ( add_noise ) then
                    ...
                    if ( stat .ne. 0_SP ) then; call scats_gen_log_error('WA_rand', call_stat); return; endif
                    ...
                    if ( stat .ne. 0_SP ) then; call scats_gen_log_error('WD_rand', call_stat); return; endif
