...
implicit none
     ...
     module procedure scats_result_write_to_file
          ...
          if ( stat .ne. 0_SP ) call scats_log_result_error('WO', file)

          ! Проверка, выделена ли памяти под массивы результата
          if ( .not. allocated(result%t) ) call scats_log_result_error('NA_t', file)
          if ( .not. allocated(result%x) ) call scats_log_result_error('NA_x', file)
